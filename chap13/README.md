...
#### 동의어란
<pre>
  테이블, 뷰, 시퀀스 등 객체 이름 대신 사용할 수 있는 다름 이름을 부여하는 객체
</pre>

#### 동의어 생성
<pre>
  <code>
    CREATE [PUBLIC] SYNONYM 동의어 리므
    FOR [사용자.][객체 이름];
  </code>
</pre>
| 요소 | 설명 |
| --- | ----- |
| PUBLIC | 동의어를 데이터베이스 내 모든 사용자가 사용할 수 있도록 설정, 생략하면 동의어를 생성한 사용자만 사용할 수 있음(선택) |
| 동의어 이름 | 생성할 동의어 이름(필수) |
| 사용자. | 생성할 동의어의 본래 객체 소유 사용자를 지정, 생략하면 현재 접속한 사용자로 지정(선택) |
